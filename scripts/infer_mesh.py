...
def main() -> None:
    ...
    scheduler = DDPMScheduler(
        steps=project_cfg.stage2.diffusion_steps,
        beta_start=project_cfg.stage2.beta_start,
        beta_end=project_cfg.stage2.beta_end,
    ).to(device)
    ...
    denoiser.load_state_dict(torch.load(args.diff_ckpt, map_location=device))
    ...
    run_inference(model, batch, str(output_path), grid_resolution=project_cfg.stage2.grid_resolution)
